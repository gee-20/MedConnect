...
export default function TransactionsLog() {
  ...
  const dummyData = [
    { id: '1', service: 'Consultation - Dr. Mushi', amount: '35,000 TZS', date: 'May 23, 2026', type: 'M-Pesa' }
  ];

  return (
    <View style={[styles.container, { backgroundColor: activeColors.background }]}>
      <Text style={[styles.headerTitle, { color: activeColors.text }]}>{t.wallet}</Text>
      ...
      <FlatList 
        ...
        renderItem={({ item }) => (
          <View style={[styles.card, { backgroundColor: activeColors.surface, borderColor: activeColors.border }]}>
            <View>
              <Text style={[styles.serviceTitle, { color: activeColors.text }]}>{item.service}</Text>
              <Text style={styles.metaDataText}>{item.date} • {item.type}</Text>
            </View>
            <Text style={[styles.valueTagText, { color: activeColors.primary }]}>{item.amount}</Text>
          </View>
        )}
      />
    </View>
...
const styles = StyleSheet.create({
  ...
  card: { padding: 16, borderRadius: 12, borderWidth: 1, flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center' },
  serviceTitle: { fontSize: 15, fontWeight: '700' },
  metaDataText: { color: '#9CA3AF', fontSize: 12, marginTop: 4 },
  valueTagText: { fontSize: 15, fontWeight: 'bold' }
});